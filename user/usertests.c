...
void copyout(char* s)
{
    ...
    for (int ai = 0; ai < sizeof(addrs) / sizeof(addrs[0]); ai++)
    {
        ...
        // ファイルの中身を読み出して、そのデータを指定したアドレスに書き込む
...
void copyinstr1(char* s)
{
    ...
    for (int ai = 0; ai < sizeof(addrs) / sizeof(addrs[0]); ai++)
    {
        ...
        // ファイル名ではなくアドレスを指定してNULL文字(\0)が見つかるまでを一つのファイルとして読み出そうとする
...
{
    // bufferを作成(サイズは128 + 1)
    // これはファイル名でxv6ではファイル名の最大の長さは128バイト
    char b[MAXPATH + 1];
    ...
    // fdの作成、つまり、bは128この'x'がある
    // x*128のファイル名を持ったファイルを開く
    ...
    // ハードリンクを作成
    // 名前bというファイルに対して、新しい名前bをつける
    // そもそもbというファイルは長さが128なのでファイルは作られていない
    ret = link(b, b);
    if (ret != -1)
    {
    ...
    // 今実行しているプロセスをxxというプログラムに変更するようにする
    // 0は終端を表す
    char* args[] = {"xx", 0};
    ret = exec(b, args);
    if (ret != -1)
    ...
    if (pid == 0)
    {
        // 1ページ分のサイズ4KiB(4096Byte)分の長さの文字列を引数に入れる
        static char big[PGSIZE + 1];
        for (int i = 0; i < PGSIZE; i++) big[i] = 'x';
        big[PGSIZE] = '\0';
        // 今回はechoでの引数にする
        // 3つのbigがあるので、4KiB * 3 = 12KiBの文字列のechoを実行しようとしている
...
void copyinstr3(char* s)
{
    ...
    sbrk(8192);
    // 現在のプロセスのメモリ末端のアドレスを取得
    uint64 top = (uint64)sbrk(0);
    // プロセスの末尾がページ単位の区切り（4096の倍数）になっているか
    if ((top % PGSIZE) != 0)
    {
        sbrk(PGSIZE - (top % PGSIZE));
    }
    // 割り当てられたメモリの最後の一バイトの次
    top = (uint64)sbrk(0);
    if (top % PGSIZE)
    {
        printf("oops\n");
    ...
    // topが割り当てられたメモリのギリギリ境界外
    // つまり、top - 1は最後のメモリ領域
    char* b = (char*)(top - 1);
    *b = 'x';

    // ファイルの削除
    // bからNULL文字が見つかるまでを一つのファイルとしてそれらを削除しようとしたが、bはメモリ境界ギリギリのところなので、bを超えたらエラーを返す必要がある
    int ret = unlink(b);
    if (ret != -1)
    {
        printf("unlink(%s) returned %d, not -1\n", b, ret);
...
void rwsbrk(char* s)
{
    ...
    // ヒープ領域を2ページ分拡張
    // このときaには拡張前の終端アドレスを確保できる
    // ヒープ領域を拡張することで、終端に他のデータがあるかもしれないから最後の1バイトという状況を作るために拡張している
    ...
    // メモリ領域の終端から1page分(4KiB)の範囲までファイルのデータを書き込む
    // ここで書き込むメモリ領域は範囲外なのでエラーを返すことを想定
    ...
    // READMEの内容をa(メモリ領域の終端)から1Page分先まで10Byte分読み込む
    // もちろん、範囲外なのでエラーが起きる
...
void truncate1(char* s)
{
    ...
    // fd1に"abcdef"の6Byteを書き込む
    // このときサイズは6Byte
    write(fd1, "abcdef", 6);

    n = read(fd3, buf, sizeof(buf));
    // 新たに追加された6Byteが読み込めると想定
    if (n != 6)
    {
    ...
    // 4Byte目から6Byte分取得する
    n = read(fd2, buf, sizeof(buf));
    // abcdまで読み取れていて、それ以降存在する文字列はefの2Byte
    if (n != 2)
    {
...
void truncate2(char* s)
{
    ...
    // truncfileを空にして、4Byte書き込む
    int fd1 = open("truncfile", O_CREATE | O_TRUNC | O_WRONLY);
    write(fd1, "abcd", 4);

    // truncfileを空にする -> filesize = 0
    int fd2 = open("truncfile", O_TRUNC | O_WRONLY);

    // fd1で1Byte(x)を書き込む
    int n = write(fd1, "x", 1);
    // fd1ではファイルサイズは4になっている
    // ファイルサイズ0のファイルに対して5Byte目にxを書き込もうとしている
    if (n != -1)
    {
